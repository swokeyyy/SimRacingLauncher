...
| iRacing UI 2025.04.14.02 | `C:\Program Files (x86)\iRacing\ui\iRacingUI.exe` | Electron: 8 processes, one owns the window titled `iRacing`. Nothing documented about close behaviour or position memory (assumption: plain close, no local unsaved state). |
...
| `monitor` | no | Monitor number from the launcher's list. Absent or `null` = launch only, do not move (a tray-only or background tool should not cost a 30-second wait on every run). Every shipped entry says `4`. |
